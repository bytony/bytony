 ...
 *文本回复
**/
class ThirdimgAction extends UserAction{
...
/*		$db=D('Thirdimg');
		...
		$info=$db->where($where)->order('createtime asc')->limit($page->firstRow.','.$page->listRows)->select();
...
*/
		...
		$this->assign('info',$info);
		$db=M('Thirdimg');
		$infos=$db->where($where)->order('createtime asc')->select();
	...
	public function add(){
		$class=M('Img')->where(array('token'=>session('token')))->select();
		if($class==false){$this->error('请先添加二级页面',U('Img/index',array('token'=>session('token'))));}
		$db=M('Img');
		$where['token']=session('token');
		$info=$db->where($where)->select();
		$this->assign('info',$info);
	...
	public function edit(){
		...
		$where['id']=$this->_get('id','intval');
		$where['uid']=session('uid');
		$res=D('Thirdimg')->where($where)->find();
		$this->assign('info',$res);
		$this->assign('res',$info);
		$this->display();
	}
	...
	public function del(){
		...
		$where['uid']=session('uid');
		if(D(MODULE_NAME)->where($where)->delete()){
			$this->success('操作成功',U('Img/index',array('token'=>session('token'))));
		}else{
			$this->error('操作失败',U('Img/index',array('token'=>session('token'))));
		}
	}

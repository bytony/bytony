...
class CouponAction extends UserAction{
	...
	//dump(session('token'));
		...
		$this->assign('group',$group);
		$this->assign('activitynum',$user['activitynum']);
		$list=M('local_Coupon')->where(array('token'=>session('token')))->select();
		echo M('local_Coupon')->getLastSql();
		$this->assign('count',M('local_Coupon')->where(array('token'=>session('token'),'type'=>3))->count());
		$this->assign('list',$list);
		$this->display();
